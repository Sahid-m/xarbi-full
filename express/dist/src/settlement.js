...
import { GAS_PRICE_MULTIPLIER, MAX_GAS_PRICE_GWEI } from "./index.js";
...
export class SettlementService {
    ...
    constructor(privateKey) {
        ...
        this.publicClient = createPublicClient({
            ...
            transport: http("https://arb-sepolia.g.alchemy.com/v2/1-dRxwkuqxEuhOE5lvNiK"),
        });
     ...
     */
    async estimateGas(tokenAddress, paymentPayload) {
        try {
            // Estimate gas limit
            ...
            const bufferedGasLimit = (gasLimit * BigInt(Math.floor(GAS_PRICE_MULTIPLIER * 100))) / 100n;
     ...
     */
    async isAuthorizationUsed(tokenAddress, authorizer, nonce) {
        try {
     ...
     */
    async settlePayment(tokenAddress, paymentPayload) {
        try {
            // Check if already used
            const isUsed = await this.isAuthorizationUsed(tokenAddress, paymentPayload.payload.from, paymentPayload.payload.nonce);
            if (isUsed) {
                return {
                    success: false,
            ...
            const maxGasPriceWei = parseGwei(MAX_GAS_PRICE_GWEI.toString());
